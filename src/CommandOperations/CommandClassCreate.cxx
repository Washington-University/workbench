...
{
    const AString name = parameters.nextString("Class Name");
    const AString derivedName = parameters.nextString("Derived From Class Name");
    
    std::vector<AString> includeFileNames;
    if (derivedName.isEmpty() == false) {
        if (derivedFromClassName.startsWith("Q")) {
            includeFileNames.push_back("<" + derivedName + ">");
        }
        else {
            includeFileNames.push_back("\"" + derivedName + "\"");
        }
