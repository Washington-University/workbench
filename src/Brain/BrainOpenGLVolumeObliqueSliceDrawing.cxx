...
#define __OLD_BRAIN_OPEN_GL_VOLUME_OBLIQUE_SLICE_DRAWING_DECLARE__
#include "BrainOpenGLVolumeObliqueSliceDrawing.h"
#undef __OLD_BRAIN_OPEN_GL_VOLUME_OBLIQUE_SLICE_DRAWING_DECLARE__
                                              ...
                                              const Plane& plane)
{
    ...
    GLdouble depthRange[2] = { 0.0, 0.0 };
    glGetDoublev(GL_DEPTH_RANGE, depthRange);
    
    /*
     * depthRange[0] is at near clipping plane
     * depthRange[1] is at far clipping plane
     * Average of the two is at the middle of the clipping planes
     */
    const double windowZ = (depthRange[0] + depthRange[1]) / 2.0;
    
    double bottomLeft[3] = { 0.0, 0.0, 0.0 };
    if (gluUnProject(viewport[0], viewport[1], windowZ,
                     modelMatrix, projMatrix, viewport,
                     &bottomLeft[0], &bottomLeft[1], &bottomLeft[2]) == GL_FALSE) {
        return;
    }
    
    double bottomRight[3] = { 0.0, 0.0, 0.0 };
    if (gluUnProject(viewport[0] + viewport[2], viewport[1], windowZ,
                     modelMatrix, projMatrix, viewport,
                     &bottomRight[0], &bottomRight[1], &bottomRight[2]) == GL_FALSE) {
        return;
    }
    
    double topLeft[3] = { 0.0, 0.0, 0.0 };
    if (gluUnProject(viewport[0], viewport[1] + viewport[3], windowZ,
                     modelMatrix, projMatrix, viewport,
                     &topLeft[0], &topLeft[1], &topLeft[2]) == GL_FALSE) {
        return;
    }
    
    double topRight[3] = { 0.0, 0.0, 0.0 };
    if (gluUnProject(viewport[0] + viewport[2], viewport[1] + viewport[3], windowZ,
                     modelMatrix, projMatrix, viewport,
                     &topRight[0], &topRight[1], &topRight[2]) == GL_FALSE) {
        return;
    }
...
//            break;
//        case VolumeSliceViewPlaneEnum::AXIAL:
//            bottomLeft[2]  -= S_EYE_OFFSET_DISTANCE;
//            bottomRight[2] -= S_EYE_OFFSET_DISTANCE;
//            topLeft[2]     -= S_EYE_OFFSET_DISTANCE;
//            topRight[2]    -= S_EYE_OFFSET_DISTANCE;
//            break;
//        case VolumeSliceViewPlaneEnum::CORONAL:
//            bottomLeft[1]  -= S_EYE_OFFSET_DISTANCE;
//            bottomRight[1] -= S_EYE_OFFSET_DISTANCE;
//            topLeft[1]     -= S_EYE_OFFSET_DISTANCE;
//            topRight[1]    -= S_EYE_OFFSET_DISTANCE;
//            break;
//        case VolumeSliceViewPlaneEnum::PARASAGITTAL:
//            bottomLeft[0]  -= S_EYE_OFFSET_DISTANCE;
//            bottomRight[0] -= S_EYE_OFFSET_DISTANCE;
//            topLeft[0]     -= S_EYE_OFFSET_DISTANCE;
//            topRight[0]    -= S_EYE_OFFSET_DISTANCE;
//            break;
//    }
    ...
    std::cout << "bottom right xyz:" << AString::fromNumbers(bottomRight, 3, ",") << std::endl;
    ...
    
    {
        ...
        if (gluUnProject(viewport[0] + viewport[2], viewport[1] + viewport[3], 1.0,
                         modelMatrix, projMatrix, viewport,
                         &xyz[0], &xyz[1], &xyz[2]) == GL_FALSE) {
            return;
        }
        std::cout << "Far plane xyz:" << AString::fromNumbers(xyz, 3, ",") << std::endl;

        if (gluUnProject(viewport[0] + viewport[2], viewport[1] + viewport[3], 0.0,
                         modelMatrix, projMatrix, viewport,
                         &xyz[0], &xyz[1], &xyz[2]) == GL_FALSE) {
            return;
        }
        std::cout << "Near plane xyz:" << AString::fromNumbers(xyz, 3, ",") << std::endl;
    }
 ...
 */
void
BrainOpenGLVolumeObliqueSliceDrawing::drawOrthogonalSlice(const VolumeSliceViewPlaneEnum::Enum sliceViewPlane,
                                                 const float sliceCoordinates[3],
                                                 const Plane& plane)
{
    const int32_t browserTabIndex = m_browserTabContent->getTabNumber();
 ...
 */
void
BrainOpenGLVolumeObliqueSliceDrawing::drawOrthogonalSliceWithCulling(const VolumeSliceViewPlaneEnum::Enum sliceViewPlane,
                                                              const float sliceCoordinates[3],
                                                              const Plane& plane)
{
    const int32_t browserTabIndex = m_browserTabContent->getTabNumber();
    ...
    for (int32_t iVol = 0; iVol < numberOfVolumesToDraw; iVol++) {
        ...
        if ( ! getVolumeDrawingViewDependentCulling(sliceViewPlane,
                                                    ...
                                                    culledLastVoxelIJK,
                                                    voxelDeltaXYZ)) {
            CaretLogSevere("BrainOpenGLVolumeObliqueSliceDrawing::getVolumeDrawingViewDependentCulling() failed.");
            continue;
        }
                                                                             ...
                                                                             const float sliceCoordinates[3])
{
    ...
    /*
     * Set look at to center of volume
     */
    m_lookAtCenter[0] = boundingBox.getCenterX();
    m_lookAtCenter[1] = boundingBox.getCenterY();
    m_lookAtCenter[2] = boundingBox.getCenterZ();
    ...
    /*
     * Just need to point the camera in the correct direction.
     * Since an orthographic projection is used, the position
     * of the camera does not matter, only its orientation.
     */
    switch (sliceViewPlane) {
        case VolumeSliceViewPlaneEnum::ALL:
        case VolumeSliceViewPlaneEnum::AXIAL:
            /* Default is looking down negative Z axis */
            break;
        case VolumeSliceViewPlaneEnum::CORONAL:
            /* Rotate about X-axis to look down positive Y-axis */
            glRotatef(-90.0, 1.0, 0.0, 0.0);
            break;
        case VolumeSliceViewPlaneEnum::PARASAGITTAL:
            /* Rotate clockwise about Y-axis to look down positive X-Axis */
    ...
    /*
     * Translate by the selected slices
     */
    const double sliceX = m_browserTabContent->getSliceCoordinateParasagittal();
    const double sliceY = m_browserTabContent->getSliceCoordinateCoronal();
    const double sliceZ = m_browserTabContent->getSliceCoordinateAxial();

    /*
     * Translate to center the slice in the window
     * NOTE: 
     */
    switch (sliceViewPlane) {
        case VolumeSliceViewPlaneEnum::ALL:
        case VolumeSliceViewPlaneEnum::AXIAL:
            glTranslatef(-boundingBox.getCenterX(),
...
//    const double sliceY = m_browserTabContent->getSliceCoordinateCoronal();
//    const double sliceZ = m_browserTabContent->getSliceCoordinateAxial();
//    glTranslated(-sliceX, -sliceY, -sliceZ);
...
//              up[0], up[1], up[2]);
    ...
    glTranslatef(userTranslation[0], userTranslation[1], userTranslation[2]);
...
//    GLdouble modelMat44[16];
//    glGetDoublev(GL_MODELVIEW_MATRIX, modelMat44);
//    Matrix4x4 modelMat;
//    modelMat.setMatrixFromOpenGL(modelMat44);
//    float modelTrans[3];
//    modelMat.getTranslation(modelTrans);
    glTranslatef(sliceCoordinates[0], sliceCoordinates[1], sliceCoordinates[2]);
    
    /*
     * Apply the oblique rotation matrix
     */
    const Matrix4x4 obliqueMatrix = m_browserTabContent->getObliqueVolumeRotationMatrix();
    double om44[16];
    obliqueMatrix.getMatrixForOpenGL(om44);
    glMultMatrixd(om44);
...
//            float translation[3];
//            mvm.getTranslation(translation);
//            glTranslatef(-translation[0], -translation[1], -translation[2]);
//            glLoadIdentity();
//            glTranslatef(translation[0], translation[1], translation[2]);
//            
////            mvm.setRotation(0.0, 0.0, 0.0);
////            mvm.getMatrixForOpenGL(mv);
////            glMultMatrixd(mv);
//            
////            float trans[3];
 ...
 */
void
BrainOpenGLVolumeObliqueSliceDrawing::drawOrthogonalSliceVoxels(const float sliceNormalVector[3],
 ...
 */
void
BrainOpenGLVolumeObliqueSliceDrawing::drawOrthogonalSliceVoxelsSingleQuads(const float sliceNormalVector[3],
                                                         const float coordinate[3],
                                                         const float rowStep[3],
                                                         const float columnStep[3],
                                                         const int64_t numberOfColumns,
 ...
 */
void
BrainOpenGLVolumeObliqueSliceDrawing::drawOrthogonalSliceVoxelsQuadIndicesAndStrips(const float sliceNormalVector[3],
 ...
 */
bool
BrainOpenGLVolumeObliqueSliceDrawing::getVolumeDrawingViewDependentCulling(const VolumeSliceViewPlaneEnum::Enum sliceViewPlane,
