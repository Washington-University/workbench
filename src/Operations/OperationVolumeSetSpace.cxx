...
OperationParameters* OperationVolumeSetSpace::getParameters()
{
    ...
    ret->setHelpText(
        ...
        "If more than one method of specifying the spacing is possible in the volume header (for nifti, this is the case), volumes are never written with inconsistent spacing information: " +
        "For each method of specifying the spacing, if it can encode the specified spacing, it will, otherwise, it will be marked as not being valid (for nifti, this means the sform " +
        "is always set as valid, and the qform is set if the specified spacing is orthogonal)."
    );
    return ret;
}
