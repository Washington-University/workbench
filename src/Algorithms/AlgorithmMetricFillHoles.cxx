...
AString AlgorithmMetricFillHoles::getCommandSwitch()
{
    return "-metric-fill-holes";
}

AString AlgorithmMetricFillHoles::getShortDescription()
{
    return "FILL HOLES IN AN ROI METRIC";
}

OperationParameters* AlgorithmMetricFillHoles::getParameters()
{
    ...
    ret->setHelpText(
        AString("Finds all connected areas that are not included in the ROI, and fills all but the largest one with ones.")
    );
    return ret;
}

void AlgorithmMetricFillHoles::useParameters(OperationParameters* myParams, ProgressObject* myProgObj)
{
    SurfaceFile* mySurf = myParams->getSurface(1);
    MetricFile* myMetric = myParams->getMetric(2);
    MetricFile* myMetricOut = myParams->getOutputMetric(3);
    AlgorithmMetricFillHoles(myProgObj, mySurf, myMetric, myMetricOut);
}

AlgorithmMetricFillHoles::AlgorithmMetricFillHoles(ProgressObject* myProgObj, const SurfaceFile* mySurf, const MetricFile* myMetric, MetricFile* myMetricOut) : AbstractAlgorithm(myProgObj)
{
    ...
    for (int col = 0; col < numCols; ++col)
    {
        ...
        {
            if (used[i] == 0 && !(roiData[i] > 0.0f))//use "not greater than" in case someone uses NaNs in their ROI
            {
        ...
        vector<float> outscratch(numNodes, 1.0f);
        ...
        if (bestArea != -1)
        {
            ...
            {
                outscratch[thisArea[i]] = 0.0f;//make it into a simple 0/1 metric, even if it wasn't before
            }
        }
...
float AlgorithmMetricFillHoles::getAlgorithmInternalWeight()
{
    return 1.0f;//override this if needed, if the progress bar isn't smooth
}

float AlgorithmMetricFillHoles::getSubAlgorithmWeight()
{
