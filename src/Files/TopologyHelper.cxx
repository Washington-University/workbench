...
TopologyHelperBase::TopologyHelperBase(const SurfaceFile* surfIn, bool sortFlag)
{
    ...
    m_maxNeigh = -1;
    m_maxTiles = -1;
    CaretArray<int> scratch(m_numNodes, -1);//mark array for added neighbors
    for (int i = 0; i < m_numNodes; ++i)
    {
        vector<int>& tileList = m_nodeInfo[i].m_tiles;
        vector<int>& vertexList = m_nodeInfo[i].m_whichVertex;
        int neighTiles = tileList.size();
        ...
        vector<int>& myNeighList = m_nodeInfo[i].m_neighbors;
        vector<int>& myEdgeList = m_nodeInfo[i].m_edges;
    ...
    CaretArray<int> scratch2(m_numTris, -1);
    if (sortFlag)
    {
        for (int i = 0; i < m_numNodes; ++i)
        {
...
void TopologyHelperBase::processTileNeighbor(vector<TopologyEdgeInfo>& tempEdgeInfo, CaretArray<int>& scratch, const int& root, const int32_t& neighbor, const int& thirdNode, const int& tile, const int& tileEdge, const bool& reversed)
{
...
void TopologyHelperBase::sortNeighbors(const SurfaceFile* mySurf, const int& node, CaretArray<int>& nodeScratch, CaretArray<int>& tileScratch)
{
    ...
    do
    {
        ...
        int node3 = m_edgeInfo[nextEdge].tiles[tileToUse].node3;
        nextEdge = -1;
        for (int i = 0; i < 3; ++i)
        {
...
int TopologyHelper::getMaximumNumberOfNeighbors() const
{
    return m_maxNeigh;
}

bool TopologyHelper::getNodeHasNeighbors(const int nodeNum) const
{
    CaretAssertVectorIndex(m_nodeInfo, nodeNum);
    return m_nodeInfo[nodeNum].m_neighbors.size() != 0;
}

const vector<int>& TopologyHelper::getNodeNeighbors(const int nodeNum) const
{
    CaretAssertVectorIndex(m_nodeInfo, nodeNum);
    return m_nodeInfo[nodeNum].m_neighbors;
}

const int* TopologyHelper::getNodeNeighbors(const int nodeNum, int& numNeighborsOut) const
{
    CaretAssertVectorIndex(m_nodeInfo, nodeNum);
    numNeighborsOut = m_nodeInfo[nodeNum].m_neighbors.size();
    return m_nodeInfo[nodeNum].m_neighbors.data();
}

int TopologyHelper::getNodeNumberOfNeighbors(const int nodeNum) const
{
    CaretAssertVectorIndex(m_nodeInfo, nodeNum);
    return m_nodeInfo[nodeNum].m_neighbors.size();
}

const vector<int>& TopologyHelper::getNodeTiles(const int nodeNum) const
{
    CaretAssertVectorIndex(m_nodeInfo, nodeNum);
    return m_nodeInfo[nodeNum].m_tiles;
}

const int* TopologyHelper::getNodeTiles(const int nodeNum, int& numTilesOut) const
{
    CaretAssertVectorIndex(m_nodeInfo, nodeNum);
    numTilesOut = m_nodeInfo[nodeNum].m_tiles.size();
    return m_nodeInfo[nodeNum].m_tiles.data();
}

const vector<int>& TopologyHelper::getNodeEdges(const int nodeNum) const
{
    CaretAssertVectorIndex(m_nodeInfo, nodeNum);
